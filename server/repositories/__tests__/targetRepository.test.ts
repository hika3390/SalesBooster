...
describe('targetRepository', () => {
  ...
  describe('findAll', () => {
    it('テナントIDで全目標を取得する', async () => {
      const mockTargets = [{ id: 1, userId: 'user1', year: 2025, month: 1, value: 1000 }];
  ...
  describe('findByUserAndPeriod', () => {
    it('ユーザーと期間で目標を取得する', async () => {
      const mockTarget = { id: 1, userId: 'user1', year: 2025, month: 6, value: 500 };
      prismaMock.target.findFirst.mockResolvedValue(mockTarget);

      const result = await targetRepository.findByUserAndPeriod('user1', 2025, 6, tenantId);

      expect(prismaMock.target.findFirst).toHaveBeenCalledWith({
        where: { tenantId, userId: 'user1', year: 2025, month: 6, periodType: 'MONTHLY', dataTypeId: null },
      });
    ...
    it('dataTypeId付きで目標を取得する', async () => {
      ...
      expect(prismaMock.target.findFirst).toHaveBeenCalledWith({
        where: { tenantId, userId: 'user1', year: 2025, month: 6, periodType: 'MONTHLY', dataTypeId: 1 },
      });
    });
  });
  ...
  describe('findByUsersAndPeriod', () => {
    it('複数ユーザーと期間で目標を取得する', async () => {
      const mockTargets = [{ id: 1, userId: 'user1' }, { id: 2, userId: 'user2' }];
      prismaMock.target.findMany.mockResolvedValue(mockTargets);

      const result = await targetRepository.findByUsersAndPeriod(['user1', 'user2'], 2025, 6, tenantId);

      expect(prismaMock.target.findMany).toHaveBeenCalledWith({
        where: { userId: { in: ['user1', 'user2'] }, year: 2025, month: 6, tenantId, periodType: 'MONTHLY' },
      });
      expect(result).toEqual(mockTargets);
    });
  });
  ...
  describe('findByUsersAndPeriodRange', () => {
    it('複数ユーザーと期間範囲で目標を取得する', async () => {
      ...
      await targetRepository.findByUsersAndPeriodRange(['user1'], 2025, 1, 2025, 3, tenantId);
  ...
  describe('upsert', () => {
    it('既存データがない場合は新規作成する', async () => {
      prismaMock.target.findFirst.mockResolvedValue(null);
      const mockCreated = { id: 1, userId: 'user1', value: 500, year: 2025, month: 6, tenantId };
      prismaMock.target.create.mockResolvedValue(mockCreated);

      const result = await targetRepository.upsert(tenantId, {
        userId: 'user1', value: 500, year: 2025, month: 6,
      });
      ...
      expect(prismaMock.target.create).toHaveBeenCalledWith({
        data: { userId: 'user1', value: 500, year: 2025, month: 6, periodType: 'MONTHLY', tenantId, dataTypeId: null },
      });
      expect(result).toEqual(mockCreated);
    });

    it('既存データがある場合は更新する', async () => {
      const existing = { id: 10, userId: 'user1', value: 300, year: 2025, month: 6 };
  ...
  describe('findByYearAndDataType', () => {
    it('年度とデータタイプで目標を取得する', async () => {
      ...
      const result = await targetRepository.findByYearAndDataType(tenantId, 2025, 1);
