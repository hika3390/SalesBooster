...
export default function GroupTargetTable({
  groups, months, groupTargets, unitLabel, onChange, onSave, saving, hasChanges, loading, calcGroupMemberTotal,
  ...
  return (
    <div>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              {months.map((m) => (
                <th key={m} className="px-2 py-2 text-center text-xs font-medium text-gray-500 border-b border-gray-200 min-w-[100px]">
                  {m}月
                </th>
              ))}
              ...
              return (
                <tr key={group.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                  ...
                  {months.map((m) => {
                    ...
                    const hasManual = manualValue !== undefined && manualValue > 0;
                    return (
                      <td key={m} className="px-1 py-1">
                        <div className="text-[10px] text-gray-400 text-right mb-0.5">
                          合計: {memberTotal > 0 ? memberTotal.toLocaleString() : '0'}
                        </div>
                        <input
                          type="number"
                          min="0"
                          value={hasManual ? manualValue : ''}
                          onChange={(e) => onChange(group.id, m, e.target.value)}
                          placeholder={memberTotal > 0 ? String(memberTotal) : '0'}
                          className={`w-full px-1.5 py-1 text-sm text-right border rounded focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400 bg-transparent ${
                            hasManual ? 'border-orange-300 bg-orange-50/50' : 'border-gray-200'
                          }`}
                        />
                      </td>
                    );
      ...
      {groups.length === 0 && !loading && (
        <div className="text-center py-8 text-gray-400 text-sm">グループが登録されていません</div>
      )}
      {loading && (
