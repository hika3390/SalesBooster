...
export default function PlaybackSettingsSection({ config, onConfigChange }: PlaybackSettingsSectionProps) {
  return (
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={config.loop}
              onChange={(e) => onConfigChange((prev) => ({ ...prev, loop: e.target.checked }))}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <Select
            ...
            onChange={(v) => onConfigChange((prev) => ({ ...prev, transition: v as TransitionType }))}
            options={[
              { value: 'NONE', label: 'なし' },
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => onConfigChange((prev) => ({ ...prev, darkMode: e.target.checked }))}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-gray-200 peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
